...
async def play_next(ctx, voice_client, queues, bot):
    ...
    try:
        ffmpeg_options = {
            'options': '-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5 -bufsize 128k -probesize 64k'
        }
        def after_playing(error):
            if error:
                ...
                logging.info(f"Finished playing {title} in guild {guild_id} with return code 0")
